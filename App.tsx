    ...
    <nav className="sticky top-0 z-50 w-full bg-bg-light/90 backdrop-blur-md border-b border-gray-100">
      <div className="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link to="/community" className="text-sm font-semibold hover:text-primary transition-colors">Community</Link>
      ...
      {isOpen && (
        <div className="fixed inset-0 top-[61px] bg-bg-light z-40 p-6 flex flex-col gap-6 md:hidden">
          ...
          <Link to="/community" className="text-2xl font-bold border-b border-gray-100 pb-2">Our Community</Link>
          <Link to="/team" className="text-2xl font-bold border-b border-gray-100 pb-2">The Team</Link>
          <Link to="/wireframes" className="text-2xl font-bold border-b border-gray-100 pb-2">Wireframes</Link>
...
const Footer: React.FC = () => (
  <footer className="bg-white border-t border-gray-100 py-12 px-4">
    <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
      <div className="col-span-1 md:col-span-2">
        ...
        <div className="flex gap-4">
          {['facebook', 'twitter', 'instagram', 'linkedin'].map((icon) => (
            <a key={icon} href="#" className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-muted hover:bg-primary hover:text-white transition-all">
               <span className="material-symbols-outlined text-[20px]">{icon === 'twitter' ? 'X' : icon}</span>
            </a>
          ))}
        </div>
      </div>
      <div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Link to="/community" className="text-sm text-muted hover:text-primary">Community Focus</Link>
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <a href="#" className="text-sm text-muted hover:text-primary">Terms of Service</a>
    ...
    <div className="max-w-6xl mx-auto mt-12 pt-8 border-t border-gray-50 text-center">
      <p className="text-xs text-muted/60">© 2024 MyKart Ghana. Reimagining retail for the Ghanaian context.</p>
    </div>
  </footer>
);
